...
\blksm{thymiovpl} (visible in big in the right margin).
...
\item An indication whether the program you are creating is well-formulated or not.
...
A program in VPL consists of one or more event-action pairs, each constructed
from an event block and an action block. For example, the pair:
\blkc{e-a-pair} causes the top light of the robot to display red when one touches the
front button on the robot.
...
\infobox{Congratulation!}{
You have created and executed your first program, that does:\\
\textbf{When one touches the forward button of Thymio, it becomes red.}
}{yellow!10}{\bcetoile}
...
\trickbox{
If you wish to recharge the robot faster, you can connect it to a main plug through a mobile phone charger featuring a micro-USB plug.
}
