...
\begin{figure}
	\subfigure[Thymio following a line of tape]{ \label{fig.tape} \includegraphics[height=0.35\textwidth]{blacktape}}
	\hfill
	\subfigure[The left sensor is off the tape and the right sensor is on the tape]{ \label{fig.one-off} \includegraphics[height=0.35\textwidth]{thymio_half_on_line}}
...
light and the event \blksm{lots-of-light} will occur. We need a line
that will cause an event to occur when there is little reflected light
\blksm{little-light}. This is easy to do by printing a black line, by painting it or by placing black
...
The first step to follow a line is to let Thymio move forward whenever \emph{both} sensors detect dark (the line) and stop whenever \emph{both} sensors detect a light surface (not the line).
The event-action pairs are shown in \cref{fig.start-stop}.

\trickbox{Make sure that you use a USB cable that is long enough (say, two
meters), so that the Thymio-II can stay connected to the computer even
as it moves.
You will find extensions in any computer shop.}


\begin{figure}
	\subfigure[Start and stop the robot]{ \label{fig.start-stop} \includegraphics[width=0.4\textwidth]{line-forward}}
	\hfill
	\subfigure[Correcting deviations]{ \label{fig.follow-line} \includegraphics[width=0.4\textwidth]{line-controller}}
...
In this program, you can set the speeds of the
...
like the one shown in \cref{fig.tape}. Try increasing the forward
...
the correct direction to find the tape again. In \cref{ch.states}
we will learn how Thymio can remember information.
}
