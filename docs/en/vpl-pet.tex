...
First, we will train the robot to obey. Normally, the robot will stay in
...
\begin{figure}
	\subfigure[Moving towards your hand]{
		\label{fig.follow-hand}
		\includegraphics[width=.4\textwidth]{likes-forward}
	}
	\hfill
	\subfigure[A bulldozer with tracks]{
		\label{fig.bull}
		\includegraphics[width=.35\textwidth]{bulldozer}
	}
	\caption{Thymio is a differential-drive vehicle}
\end{figure}
...
different values. In the block \blksm{differential}, the left slider has
been set for fast speed backwards, while the right slider has been set
for fast speed forwards. The result is that the robot will make a left turn on itself, as indicated by the small image of the robot within the motor action block.
...
\trickbox{The motor action block shows an animation of Thymio's movement when you move the sliders.}
...
{\small As explained in \cref{c.moving}, a lot of light will be reflected
...
{
Experiment with the sensors. Instead of using sensors 0 and 4 as until now:
\begin{itemize}[noitemsep,nosep,leftmargin=*]
...
\end{itemize}
{\small The front horizontal sensors are numbered 0, 1, 2, 3, 4 from the left of the robot to its right.
...
\Cref{fig.textcode} shows the program where the pet likes you and follows you around along with the text translation at the right.
...
It starts by testing the sensor number 2 (front center) as we see with \p{prox.horizontal[2]}.
...
Every \p{if} \ldots \ \p{then} \ldots \ \p{end} block tests a specific sensor and executes or not the associated action, in fonction of the result of the test.
...
\end{enumerate}
Finally, once Thymio has read all these sensors, it waits the next event \p{prox} et starts over these tests, indefinitely.

\trickbox{Experiment with moving the sliders on the motor action blocks. You will
