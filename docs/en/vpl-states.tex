...
tap block together with an indication of the state: \blkc{tap-turn-on}
...
orange, meaning that the robot's light is on: \blkc{tap-turn-off}
...
\trickbox{When a program is run, the initial state is
...
Also, if for instance you have two different things you want to encode in the state, and each of them have two possible values, you can use two quarters independently.
...
\trickbox{
You will have to experiment with the position of the
