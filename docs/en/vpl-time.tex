...
\blkc{start-turn} and for turning away when your hand is detected:
\blkc{turn-away}
...
\blksm{action-timer}. Normally, we set an alarm clock to an absolute time,
...
event block \blksm{event-timer} shows a ringing alarm clock.

The event-action pair to turn the robot anew to
the right when the timer runs down is: \blkc{turn-back}
...
\trickbox{
The concept of timer is extremely useful in many situations in robotics, you will see that quickly when doing your own behaviors for Thymio.
}