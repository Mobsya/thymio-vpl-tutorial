...
Si le carré est blanc avec un bord rouge \blksm{center-prox}, un événement est déclanché
si beaucoup de lumière est réfléchie.

Si le carré est noir \blksm{center-no-prox}, un événement est déclanché si peu de lumière est réfléchie.
...
\blksm{event-tap} \textbf{Tappe}. Un événement est déclanché lorsqu'on donne une petite tappe au Thymio.

\bigskip\bigskip\bigskip

\blksm{event-tap-advanced} \textbf{Tappe, mode avancé}.
...
L'événement sera alors déclanché 10 fois par secondes, peu importe les valeurs mesurées par les capteurs.}

\informationbox{Événements boutons rapidement répétés}{
La même remarque vaut pour les événements boutons, avec la différence que ceux-ci sont déclanchés
20 fois par seconde.}
