...
\chap{Apprendre le langage AESL à partir des programmes VPL}\label{c.next}
...
Lorsqu'un événement tappe est déclanché, la \emph{fonction d'interface} \p{leds.top} est \emph{appelée}.
...
L'événement VPL frapper dans les mains correspond à l'événement \p{mic} (une forme abbrégée de 
microphone).
...
à gauche.
La variable \p{mic.threshold} est prédéfini pour le robot Thymio.
...
signifient: \p{si} la valeur de la variable \p{button.forward} (\emph{button.forward} signifie
<<bouton.avant>> en anglais) est un,
alors exécute les actions écrites entre les mots-clés \p{if} (qui signifie <<si>> en anglais)
...
Dans ce programme, il y a cinq structures introduites par \p{if}, une pour chaque bouton.
Une, deux ou trois actions sont exécutées si l'expression dans une structure \p{if} est vraie.
...
Ainsi, nous avons déjà vu les mots-clés \p{onevent}, \p{if}, \p{then} et \p{end}
...
en plus de l'événement \p{buttons} qui est déclanché chaque fois qu'un bouton est pressé ou relâché.
Nous pourions aussi implémenter ce programme avec de multiples événements et sans structures
\p{if} ni variables bouton:
...
avec plusieurs structures \p{if}.
...
L'événement est appelé \p{prox} (une version abrégée de <<proximité>>).
...
AESL.}
Le programme \p{cats.aesl} dans l'archive des projets pour AESL contient l'instruction suivante:
...
L'instruction suivante est une boucle \p{for} (de l'anglais <<pour>>) dont la signification est: 
...
Dans le programme AESL correspondant, la structure \p{if} vérifie si le bouton central a été touché,
...
L'intensité de chaque LED peut être contrôllée en donnant une valeur entre 0 (LED éteinte)
