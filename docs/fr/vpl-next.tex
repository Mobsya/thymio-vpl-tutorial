...
signifient: \p{si} (<<\,if\,>> signifie <<\,si\,>> en anglais) la valeur de la variable \p{button.forward} (\emph{button.forward} signifie
<<\,bouton.avant>> en anglais) est un,
alors exécute les actions écrites entre les mots-clés \p{then} (qui signifie <<\,alors\,>> en anglais)
