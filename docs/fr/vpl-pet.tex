...
\begin{figure}
	\subfigure[Thymio avance vers votre main]{
		\label{fig.follow-hand}
		\includegraphics[width=.4\textwidth]{likes-forward}
	}
	\hfill
	\subfigure[Un bulldozer à chenilles]{
		\label{fig.bull}
...
% \begin{center}
% \gr{bulldozer}{0.35}
% \caption{Un bulldozer qui utilise aussi la direction différentielle}\label{fig.bull}
% \end{center}
% \end{figure}
...
Dans le bloc \blksm{differential}, le \textit{slider} gauche indique une grande vitesse en arrière, alors que le \textit{slider} droite indique une grande vitesse en avant.
...
\trickbox{Le bloc action moteur s'anime dès que vous régler les \textit{sliders} pour vous donner une idée du mouvement de Thymio!}
...
Qu'arrive-t-il si vous changez l'ordre des paires d'événement-action utilisée à l'exercice précédent ?
}
...
Ouvrez le programme du compagnon qui vous aime et échangez l'association des événements avec les actions.
Détecter un obstacle avec le capteur gauche fait tourner le robot à droite, et détecter un obstacle avec le capteur droite fait tourner le robot à gauche, comme montré sur la \cref{fig.hates}.
...
\trickbox{Expérimentez avec les \textit{sliders} des blocs d'action moteur.
Vous verrez les vitesses visées pour les moteurs (\p{moter.X.target}) changer par pas de 50 dans l'intervalle $-$500 à 500.
