...
Mais regardez votre ordinateur, normalement, il n'a qu'un seul bouton \blksm{power-button} pour l'allumer ou l'éteindre.
...
Depuis l'état \bu{éteint}, le robot peut aller dans l'état \bu{allumé} et revenir, mais seulement en suivant les instruction sur les flèches.
...
Dans la première paire événement-action, l'événement est composé du bloc détection de choc avec une indication d'état : \blkc{tap-turn-on}
...
Ce quartier est coloré en orange, donc le robot est allumé : \blkc{tap-turn-off}
...
Par exemple, dans \blksm{states}, les quartiers en haut à gauche et en bas à droite sont allumés, le quartier en haut à droite est éteint, et le quartier en bas à gauche n'est pas prise en compte.
...
\trickbox{Lorsqu'un programme démarre, l'état initial est toujours 
...
Aussi, si par exemple vous avez deux choses différentes à encoder dans l'état, et que chacune d'elle a deux valeurs possible, vous pouvez utiliser deux quartiers indépendemment.
C'est pourquoi la capacité d'\emph{ignorer} une quartier est très utile !
...
\trickbox{
Il vous faudra expérimenter avec la position de la souris.
