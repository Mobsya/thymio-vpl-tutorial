...
\Cref{fig.music} shows two event-actions pairs that respond with a tune
...
Write a program that will enable you to send a message is
\href{http://en.wikipedia.org/wiki/Morse_code}{Morse code}. Letters in
Morse code are encoded in sequences of long tones (\emph{dashes}) and
short tones (\emph{dots}). For example, the letter \emph{V} is encoded
by three dots followed by one dash.}
...
\trickbox[Information]{In a noisy environment you may not be able to use this
...
\vspace{.5em}\\
Write a program that does the opposite: the robot starts when you touch a
button and stops when you clap your hands.
}
...
the following event-actions pair causes the top lights to turn on when
...
wall.
\vspace{.5em}\\
Make sure that the robot \textbf{moves slowly} so that it
