...
These projects use the \blksm{event-clap} event to change states and
...
``fingers'' (\bu{off} and \bu{on}) so base 2 arithmetic is used in
...
\bu{off} (white) for none and \bu{on} (orange) for one, and let the
