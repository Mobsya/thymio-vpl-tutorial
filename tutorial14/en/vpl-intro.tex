...
\Cref{fig.front} shows the front and top of Thymio II. On the top you
...
on the icon \blksm{thymiovpl} on your computer.

\importantbox[Images of a button]{When a small image of a button appears
...
\item The text translation of the program.
\end{enumerate}
...
\trickbox[To go further]{ When you construct a program using VPL, the
text program that will be loaded into the robot appears in the right
part of the window. If you are curious and wish to understand this
language, you can read the
\href{https://aseba.wikidot.com/en:thymiotutoriel}{text mode tutorial}
(\url{http://aseba.wikidot.com/en:thymiotutoriel}). }

\newpage

\sect{Write a program}

When you start VPL, a blank program area is displayed; if, after having
built a piece of program, you wish to clear the content of the program
area, click \blksm{new} (\bu{New}).
...
\importantbox[Meaning of an event-actions pair]{\textbf{When the event
occurs, the associated action(s) are run.}}

Let us construct an event-actions pair. An outline of a pair is
displayed in the program area: \blkc{event-action-pair-empty} The left,
gold-colored, square is the space for the event; the right,
blue-colored, square is the space for the first (or only) action. To
bring a block to the program area from the sides (areas 4 and 5 of
\cref{fig.vplgui}), press and hold the left mouse button and drag the
block to a square. When the block is in place, release the mouse button.

Start by bringing the button event block \blksm{event-buttons} into the
gold square. Now, bring the top color action block
\blksm{action-colors-up} into the blue square. You have constructed an
event-actions pair!
...
red: \blkc{forward} This specifies that \textbf{an event will occur when
...
left. The color will be all red with no blue nor green: \blkc{red}
...
\informationbox{Congratulations!}{You have created and executed your
...
\item To delete an event-actions pair, click \blksm{x} at the top-right
of the pair.

\item To add an event-actions pair, click \blksm{plus} available below
an existing pair.
...
button \blksm{open} (\bu{Open}) and select the program you want to open, for
...
\informationbox{The VPL toolbar}{Appendix~\ref{a.toolbar} contains a
