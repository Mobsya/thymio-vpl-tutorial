...
\bu{off}. The switch includes a small green light which indicates the
current state.
...
\importantbox[Blocks in advanced mode]{The block for the tap event is
different in advanced mode, because it is also used for accelerometer
events as described in Chapter~\ref{ch.angles}. The sensor blocks are
also different as described on page \pageref{p.proximity-sensitivity}.}
...
together with an indication of the state \blksm{state-filter}:
\blkc{tap-turn-on}
...
\bu{off}. Therefore, we need to add a second action to each
event-actions pair. This is the \emph{state} action block
\blksm{action-states} (\cref{fig.turn-on-off}).
...
For example, in \blksm{states}, the left-front and right-rear quarters
are on, the right-front one is off and the left-rear one is not taken
...
\importantbox{The current state of the robot is displayed in the circle
LEDs on the top of the robot. \cref{fig.state-leds} shows the robot in
the state \bu{(on, on, on, on)}.}
...
\blkc{mouse3}
When in state \bu{search right} the mouse is detected (only) by the
...
\informationbox{Setting the thresholds of the sensors}{In basic mode,
the thresholds of the sensors are fixed. For the horizontal sensors, a
value above 2000 means that a lot of light is reflected and an event
will occur if the corresponding square is white, while a value below
1000 means that little light is reflected and an event will occur if the
corresponding square is black. For the bottom sensors, the values are
300 and 150. In advanced mode, the thresholds can be set. There is a
pair of sliders in each block. The top slider sets the threshold above
which the white event occurs and the bottom slider sets the threshold
below which the black event occurs.\label{p.proximity-sensitivity}}
