...
from the tutorial
\href{https://aseba.wdfiles.com/local--files/en:thymioprogram/thymio-vpl-tutorial-en.pdf}%
{Thymio-II Robot and the Aseba/VPL Environment} and explains the
...
The VPL programs are described in
\textit{Thymio-II Robot and the Aseba/VPL Environment}:\\
\url{https://aseba.wdfiles.com/local--files/en:thymioprogram/thymio-vpl-tutorial-en.pdf}\\
and the programs are in the archive:\\
...
be controlled. It is very important that you print out this document or keep
it open in a browser window until you are completely familiar with the
interface. You will also want to print out the ``cheat sheet'':\\  
\url{https://aseba.wdfiles.com/local--files/en:thymioprogram/ThymioCheatSheet.pdf}\\
which is a diagram of the Thymio robot that identifies the parts of the
robot (for example, the center horizontal proximity sensor is numbered 2).   
...
\item Version~1.4 allows multiple actions to be associated with a single
event, as shown in the left image below. In Version~1.3, you need to use
two event-action pairs that have the same event but different actions.

\begin{center}
\begin{tabular}{c@{\extracolsep{.1\textwidth}}c}
\raisebox{12ex}{\includegraphics[width=.4\textwidth]{colors2-1-4}} &
\includegraphics[width=.32\textwidth]{colors2-1-3} 
\end{tabular}
\end{center}


\item The meaning of the footnotesize squares in the blocks for the proximity
sensors has changed:

\begin{itemize}

\item In Version~1.4 \emph{black} means that an event occurs when there
is little reflected light (there is no object in front of the sensor),
while \emph{white} means that an event occurs when there is a lot
reflected light (an object is detected in front of the sensor).

\item In Version~1.3, the colors red and white are used and white has
the opposite meaning. \emph{White} means that an event occurs when there
is little reflected light (there is no object in front of the sensor),
while \emph{red} means that an event occurs when there is a lot
reflected light (an object is detected in front of the sensor).
...
Here is the program \p{whistles.aesl} from Chapter~6 of the VPL
...
The variable \p{mic.threshold} is predefined in Aseba.
...
\p{colors-multiple.aesl}) changes the colors of the top and bottom LEDs
...
The program \p{likes.aesl} in Chapter~4 of the VPL tutorial causes the
...
\p{if}-statements. The values of the motor variables
...
that the \p{prox} event occurs with frequency \emph{10 Hz}.}
...
projects.} The program \p{cats.aesl} in
...
(milliseconds) should be assigned to \p{timer.period[0]}. In VPL, you
...
program \p{count-to-two.aesl} from Chapter~9, the state is set to 0 when
...
\begin{center}
\begin{tabular}{ll}
\raisebox{16ex}{\includegraphics[width=.4\textwidth]{two-clap}} &
\begin{minipage}[b]{.5\textwidth}
\begin{footnotesize}
\begin{verbatim}
  onevent mic
    if state[0] == 0 and state[1] == 0 and
       state[2] == 0 and state[3] == 0 then
      state[0] = 1
      state[1] = 0
      state[2] = 0
      state[3] = 0
    end
    if state[0] == 1 and state[1] == 0 and
       state[2] == 0 and state[3] == 0 then
      state[0] = 0
      state[1] = 0
      state[2] = 0
...
\p{state[...]*32} which multiplies the value of a component of the array
